...
@section('title', 'All Ebook - Ohm Pharmaceuticals')

@section('content')
    @include('admin.includes.message')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Ohm Ebook ({{ $ebook->total() }})</h5>
            <small class="text-muted float-end">
                <a class="btn btn-primary" href="{{ route('ebook.create') }}"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
            ...
            @if (!$ebook->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($ebook as $key => $blg)
                            <tr>
                                <td><strong>{{ $key + $ebook->firstItem() }}</strong></td>
                                ...
                                <td>
                                    <a class="btn btn-sm btn-success" href="{{ route('ebooksingle', $blg['slug']) }}"
                                        style="float: left;margin-right: 5px;" target="_blank"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a class="btn btn-sm btn-primary" href="{{ route('ebook.edit', $blg->id) }}"
                                        style="float: left;margin-right: 5px;"><i class="fa-solid fa-pen-to-square"></i></a>

                                    <form class="delete-form" action="{{ route('ebook.destroy', $blg->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
